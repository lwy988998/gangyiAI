...
`gangyiAI-setup-v0.1.1-x64.exe`，安装后填写 OpenAI Chat Completions 兼容接口地址、API Key 和模型名称即可使用。
首次发布版本尚未进行代码签名，Windows SmartScreen 可能显示未知发布者提示。

安装包同页提供 `SHA256SUMS.txt`，可使用以下命令校验下载文件：

```powershell
Get-FileHash .\gangyiAI-setup-v0.1.1-x64.exe -Algorithm SHA256
```
...
```text
dist\installer\gangyiAI-setup-v0.1.1-x64.exe
```
