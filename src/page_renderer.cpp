...
<main class="learn-app-page min-h-screen bg-[#f5f9ff] text-slate-950">
  <section class="mx-auto w-full max-w-6xl px-4 py-6 sm:px-6">
    ...
    const std::string script = R"HTML(<script>(function(){let anonymousId=)HTML" + jsString(anonymousId) + R"HTML(,authenticated=)HTML" + (authenticated ? "true" : "false") + R"HTML(;if(!authenticated&&!anonymousId){anonymousId=localStorage.getItem('gangyi-anonymous-id')||crypto.randomUUID();localStorage.setItem('gangyi-anonymous-id',anonymousId);location.replace('/my-courses?anonymousId='+encodeURIComponent(anonymousId));return}const message=document.getElementById('course-message');document.querySelectorAll('.delete-course').forEach(button=>button.addEventListener('click',async()=>{if(!confirm('确定删除这门课程吗？学习进度和课程内容将一并删除。'))return;button.disabled=true;try{const query=anonymousId?'?anonymousId='+encodeURIComponent(anonymousId):'';const response=await fetch('/api/my-courses/'+encodeURIComponent(button.dataset.courseId)+query,{method:'DELETE'});if(!response.ok)throw new Error();location.reload()}catch(_){button.disabled=false;message.textContent='删除失败，请稍后重试。'}}))})();</script>)HTML";
