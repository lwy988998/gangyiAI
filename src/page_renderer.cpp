...
function failed(block,message){const id={overview:'learn-content-summary',steps:'learn-steps',examples:'learn-examples',practice:'learn-practice',quiz:'learn-quiz',assessment:'learn-checkpoint'}[block],box=$(id);box.innerHTML='<div class="rounded-2xl border border-rose-100 bg-rose-50 p-4 text-sm text-rose-700"><p>'+E(labels[block])+'已自动尝试 3 次，仍未生成。</p><p class="mt-1">'+E(message)+'</p><button type="button" data-retry-block="'+block+'" class="mt-3 rounded-xl bg-sky-700 px-4 py-2 font-semibold text-white">重试此板块</button></div>';updateStatus()}
...
async function request(block,retry=false){const url=base()+'&block='+encodeURIComponent(block)+(params.get('regenerate')==='1'?'&regenerate=1':'')+(retry?'&retry=1':'');const response=await fetch(url);let data={};try{data=await response.json()}catch(_){}if(!response.ok||data.ok===false)throw new Error(data.error||'AI 生成失败');apply(block,data.content);references(data.references);return data}
async function start(){const containers={steps:'learn-steps',examples:'learn-examples',practice:'learn-practice',quiz:'learn-quiz',assessment:'learn-checkpoint'};$('learn-loading').classList.add('hidden');$('learn-content').classList.remove('hidden');$('quiz-submit').disabled=true;$('complete-lesson').disabled=true;Object.values(containers).forEach(waiting);$('learn-content-summary').textContent='AI 正在生成本课概览…';try{const response=await fetch(base()),saved=await response.json();if(!response.ok||saved.ok===false)throw new Error(saved.error||'课程读取失败');$('learn-phase-name').textContent=saved.phaseName||params.get('phaseName')||'当前阶段';$('learn-topic-name').textContent=saved.topicTitle||params.get('topic')||'当前主题';if(params.get('regenerate')!=='1')Object.entries(saved.blocks||{}).forEach(([block,data])=>apply(block,data));references(saved.references)}catch(error){failed('overview',error.message);return}for(const block of order){if(blocks[block])continue;try{await request(block)}catch(error){failed(block,error.message)}}}
document.addEventListener('click',event=>{const retry=event.target.closest('[data-retry-block]');if(retry){const block=retry.dataset.retryBlock;retry.disabled=true;retry.textContent='AI 正在重试…';request(block,true).catch(error=>failed(block,error.message));return}const toggle=event.target.closest('.step-toggle');if(!toggle)return;const index=+toggle.dataset.i;state.steps=state.steps||{};state.steps[index]=!state.steps[index];toggle.textContent=state.steps[index]?'✓':'';updateStepCount();localStorage.setItem('gy:'+C+':'+P+':'+T,JSON.stringify(state));fetch('/api/learning-step-progress',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({courseId:C,phaseIndex:+P,stepIndex:index,stepTitle:lesson.lessonSteps[index].title,status:state.steps[index]?'understood':'unset'})})});
