...
<style>.home-school-logo{width:80px;height:80px}@media(min-width:640px){.home-school-logo{width:112px;height:112px}}@media(min-width:1024px){.home-school-logo{width:128px;height:128px}}</style>
<main class="min-h-screen bg-[radial-gradient(circle_at_top,rgba(219,234,254,0.9),rgba(248,251,255,0.98)_44%,rgba(255,255,255,1))] text-slate-950">
  <section class="mx-auto flex min-h-[calc(100vh-3.25rem)] w-full max-w-6xl flex-col items-center px-4 pb-8 pt-3 text-center sm:px-6 sm:pt-5">
    <img src="/school-logo.png" alt="柳州市钢一中学校徽" width="680" height="680" fetchpriority="high" class="home-school-logo shrink-0 object-contain"><p class="mt-2 text-xs font-semibold uppercase tracking-[0.14em] text-sky-700">柳州市钢一中学</p><h1 class="mt-1 max-w-3xl text-2xl font-bold tracking-tight text-sky-700 sm:text-4xl">柳州市钢一中学2629班定制AI</h1><p class="mt-1 max-w-xl text-sm leading-6 text-slate-600 sm:text-base">把学习目标变成可以一步步完成的课程。</p>
    <form id="goal-form" class="mt-4 w-full max-w-3xl rounded-2xl border border-slate-200 bg-white p-2 text-left shadow-lg shadow-sky-900/10" action="/plan" method="get"><input id="goal-image" type="file" accept="image/*" class="hidden"><input id="goal-mode" type="hidden" name="mode" value="deep">
...
<main class="learn-app-page min-h-screen bg-[#f5f9ff] text-slate-950"><section class="mx-auto w-full max-w-6xl px-4 py-6 sm:px-6"><div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div><p class="text-sm font-semibold text-sky-700">微课程 · 学习中</p><h1 id="learn-title" class="mt-2 text-3xl font-bold tracking-tight sm:text-4xl">正在生成本节微课程</h1><p id="learn-summary" class="mt-3 max-w-3xl leading-7 text-slate-600">AI 正在根据课程目标和当前分支生成专属讲解、练习与测验。</p></div><aside class="grid gap-2 rounded-3xl border border-sky-100 bg-white p-4 shadow-sm sm:min-w-72 sm:max-w-sm"><div class="flex items-start justify-between gap-3 text-sm"><span class="shrink-0">阶段</span><b id="learn-phase-name" class="max-w-56 break-words text-right">)HTML" + htmlEscape(phaseName.empty() ? "正在确认阶段" : phaseName) + R"HTML(</b></div><div class="flex items-start justify-between gap-3 text-sm"><span class="shrink-0">主题</span><b id="learn-topic-name" class="max-w-56 break-words text-right">)HTML" + htmlEscape(topic.empty() ? "正在确认主题" : topic) + R"HTML(</b></div><div class="h-2 overflow-hidden rounded-full bg-slate-200"><i id="learn-bar" class="soft-progress-fill block h-full w-[12%] rounded-full bg-sky-600"></i></div><small id="learn-save-status">正在生成课程内容</small></aside></div><section class="mt-6 rounded-3xl border border-sky-100 bg-white p-4 shadow-sm sm:p-6"><div id="learn-loading" class="p-8 text-center"><div class="mx-auto h-10 w-10 animate-spin rounded-full border-4 border-sky-100 border-t-sky-600"></div><p class="mt-4 font-semibold">正在生成与当前主题对应的讲解</p><p class="mt-2 text-sm text-slate-500">系统会自动检查内容质量；未通过时会根据原因重新生成。</p></div><div id="learn-content" class="hidden space-y-6"><section class="grid gap-4 lg:grid-cols-[1.3fr_0.9fr]"><article class="rounded-2xl border border-sky-100 bg-sky-50/70 p-5"><p class="text-sm font-semibold text-sky-700">本课概览</p><h2 id="learn-content-title" class="mt-2 text-2xl font-bold"></h2><p id="learn-content-summary" class="mt-3 leading-7 text-slate-600"></p></article><article class="rounded-2xl border border-slate-200 p-5"><p class="text-sm font-semibold">关键概念</p><div id="learn-key-concepts" class="mt-3 flex flex-wrap gap-2"></div></article></section><section class="grid gap-4 lg:grid-cols-2"><article class="rounded-2xl border border-sky-100 p-5"><div class="flex justify-between"><p class="text-sm font-semibold text-sky-700">理解步骤</p><small id="step-count"></small></div><div id="learn-steps" class="mt-4 space-y-3"></div></article><article class="rounded-2xl border border-sky-100 p-5"><p class="text-sm font-semibold text-sky-700">示例与练习</p><div id="learn-examples" class="mt-4 space-y-3"></div><div id="learn-practice" class="mt-4 space-y-3"></div></article></section><section class="grid gap-4 lg:grid-cols-2"><article class="rounded-2xl border border-sky-100 p-5"><div class="flex justify-between"><p class="text-sm font-semibold text-sky-700">小测验</p><small>答对 70% 即通过</small></div><div id="learn-quiz" class="mt-4 space-y-3"></div><button id="quiz-submit" type="button" class="mt-4 min-h-11 w-full rounded-xl bg-sky-700 px-4 text-sm font-semibold text-white">提交测验</button><p id="quiz-result" class="mt-3 text-sm font-semibold"></p></article><article class="rounded-2xl border border-sky-100 p-5"><p class="text-sm font-semibold text-sky-700">完成检查</p><div id="learn-checkpoint" class="mt-3 space-y-2 text-sm leading-7"></div><div id="learn-mistakes" class="mt-4 space-y-2 text-sm leading-7"></div><button id="complete-lesson" type="button" class="mt-5 min-h-11 w-full rounded-xl bg-emerald-600 px-4 text-sm font-semibold text-white">完成本节并保存进度</button></article></section><section class="rounded-2xl border border-slate-200 bg-slate-50 p-5"><div class="flex items-center justify-between gap-3"><div><p class="text-sm font-semibold">参考资料</p><p id="learn-resource-summary" class="mt-1 text-xs leading-5 text-slate-500"></p></div><button id="regenerate" type="button" class="rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm font-semibold">换一版讲解</button></div><div id="learn-references" class="mt-3 grid gap-3 md:grid-cols-2"></div></section></div></section></section></main>)HTML";
    ...
    const std::string learningContextBridge = R"HTML(<script>(()=>{const params=new URLSearchParams(location.search),nativeFetch=window.fetch.bind(window),status=document.getElementById('learn-save-status'),bar=document.getElementById('learn-bar');let stage=0;const timer=setInterval(()=>{const states=[['正在读取课程分支',18],['正在检索真实资料',32],['AI 正在生成专属讲解',52],['正在检查内容质量',72],['未通过时自动重新生成',84]];const current=states[Math.min(stage++,states.length-1)];if(status&&document.getElementById('learn-content')?.classList.contains('hidden'))status.textContent=current[0];if(bar&&document.getElementById('learn-content')?.classList.contains('hidden'))bar.style.width=current[1]+'%'},6000);window.addEventListener('pagehide',()=>clearInterval(timer));window.fetch=(input,init={})=>{const url=typeof input==='string'?input:'';if(url.startsWith('/api/learn?')){const content=document.getElementById('learn-content'),refs=document.getElementById('learn-references');if(content&&!content.classList.contains('hidden')&&refs&&refs.children.length)return Promise.reject(new Error('resource data already loaded'));const request=new URL(url,location.origin);['phaseName','topic','goal','mode'].forEach(key=>{const value=params.get(key);if(value&&!request.searchParams.has(key))request.searchParams.set(key,value)});const pending=nativeFetch(request.pathname+request.search,init);return Promise.race([pending,new Promise((_,reject)=>setTimeout(()=>reject(new Error('学习内容生成超时，请重试')),150000))])}return nativeFetch(input,init)}})()</script>)HTML";
    const std::string resourceEnhancement = R"HTML(<script>(()=>{const esc=v=>{const d=document.createElement('div');d.textContent=v??'';return d.innerHTML};let loaded=false;const render=()=>{if(loaded)return;const content=document.getElementById('learn-content');if(!content||content.classList.contains('hidden'))return;loaded=true;const p=new URLSearchParams(location.search);const query=new URLSearchParams({courseId:p.get('courseId')||'',phaseIndex:p.get('phaseIndex')||'0',topicIndex:p.get('topicIndex')||'0'});fetch('/api/learn?'+query.toString()).then(r=>r.json()).then(x=>{const summary=document.getElementById('learn-resource-summary');if(summary)summary.textContent=x.resourceSummary||'公开学习资料';const box=document.getElementById('learn-references');if(!box)return;const refs=Array.isArray(x.references)?x.references:[];box.innerHTML=refs.length?refs.map(v=>{const url=/^https?:\/\//i.test(v.url||'')?esc(v.url):'#';return '<a class="block rounded-2xl border border-sky-100 bg-white p-4 transition hover:border-sky-300" target="_blank" rel="noreferrer" href="'+url+'"><div class="flex items-center justify-between gap-2"><span class="text-xs font-semibold text-sky-700">'+esc(v.source||v.type||'联网资料')+'</span><span class="text-xs text-slate-500">'+esc(v.difficulty||'入门')+'</span></div><h3 class="mt-2 font-semibold text-slate-950">'+esc(v.title||'参考资料')+'</h3><p class="mt-2 text-sm leading-6 text-slate-600">'+esc(v.description||'点击打开真实学习资料。')+'</p></a>'}).join(''):'<p class="text-sm text-slate-500">暂未检索到公开资料，请稍后点击“换一版讲解”。</p>'}).catch(()=>{})};const timer=setInterval(()=>{render();if(loaded)clearInterval(timer)},250)})()</script>)HTML";
    const std::string learningStatusEnhancement = R"HTML(<script>(()=>{const content=document.getElementById('learn-content'),status=document.getElementById('learn-save-status'),bar=document.getElementById('learn-bar');if(!content||!status||!bar)return;const update=()=>{if(content.classList.contains('hidden'))return;const steps=[...document.querySelectorAll('.step-toggle')],practice=[...document.querySelectorAll('.practice-toggle')],total=steps.length+practice.length,done=steps.filter(x=>x.textContent.trim()==='✓').length+practice.filter(x=>x.checked).length,percent=total?Math.round(done/total*100):0;bar.style.width=percent+'%';status.textContent='已生成 '+steps.length+' 个讲解步骤 · '+practice.length+' 项练习 · 学习进度 '+percent+'%'};new MutationObserver(update).observe(content,{attributes:true,attributeFilter:['class'],childList:true,subtree:true});document.addEventListener('click',e=>{if(e.target.closest('.step-toggle'))setTimeout(update)});document.addEventListener('change',e=>{if(e.target.matches('.practice-toggle'))setTimeout(update)});update()})()</script>)HTML";
    ...
    return document("学习中 - 钢一定制AI", body, bridge + identityBridge + learningContextBridge + script + resourceEnhancement + learningStatusEnhancement + progressFeedbackEnhancement + lessonNavigationEnhancement + backNavigation + chatEnhancement);
}
...
<main class="min-h-screen bg-[#f5f9ff] text-slate-950"><section class="mx-auto w-full max-w-6xl px-4 py-6 sm:px-6 lg:py-10">
  <header class="rounded-3xl border border-sky-100 bg-white p-6 shadow-sm shadow-sky-900/5 sm:p-8"><p class="text-sm font-semibold text-sky-700">AI 对话</p><h1 class="mt-2 text-3xl font-bold tracking-tight sm:text-4xl">有什么问题，直接问我</h1><p class="mt-3 text-base leading-7 text-slate-600">由 DeepSeek V4 Flash 直接回答，不经过课程生成链路。</p></header>
